...
// }) {
//   return _NirmaanYoutubePlayerMobileMac(
//     url: url,
...
// }
//
// class _NirmaanYoutubePlayerMobileMac extends StatefulWidget {
//   final String url;
...
//
//   const _NirmaanYoutubePlayerMobileMac({
//     required this.url,
...
//
//   @override
//   State<_NirmaanYoutubePlayerMobileMac> createState() =>
//       _NirmaanYoutubePlayerMobileMacState();
// }
//
...
//   bool _isChangingFullscreenFromButton = false;
...
//     _rotationDebounce = Timer(const Duration(milliseconds: 350), () {
//       if (!mounted) return;
//       if (_isChangingFullscreenFromButton) return;
//       final view = View.of(context);
//       final size = view.physicalSize;
//       final isLandscape = size.width > size.height;
//       if (isLandscape != _isFullscreen) _setFullscreenFromRotation(isLandscape);
//     });
//   }
//
//   // ── Local HTTP server ─────────────────────────────────────────────────────
...
//
//         // Serve MP4 bytes with Range support for seeking
...
//           // Prevent caching / saving by the browser layer
...
//     } catch (e) {
//       if (!mounted) return;
//       setState(() => _errorMessage = 'Local player server failed:\n\n$e');
//     }
//   }
//
...
//   }
//
//   // ── Offline HTML5 player — full Nirmaan controls, download blocked ────────
...
//
// /* ── Constants ─────────────────────────────────────────────────────────────── */
// const SOURCE      = 'nirmaan-youtube-mobile-macos';
// const SEEK_STEP   = 10;
...
//   Future<void> _setFullscreenFromRotation(bool value) async {
//     try {
//       _isFullscreen = value;
//       if (_isFullscreen) {
...
//       await _syncFullscreenToHtml();
//     } catch (e) {
//       debugPrint('AUTO ROTATION FULLSCREEN ERROR => $e');
//     }
//   }
//
//   Future<void> _toggleFullscreen() async {
//     try {
//       _isChangingFullscreenFromButton = true;
...
//       _isChangingFullscreenFromButton = false;
//     } catch (e) {
//       _isChangingFullscreenFromButton = false;
//       debugPrint('MOBILE/MAC FULLSCREEN ERROR => $e');
//     }
//   }
//
...
//       if (value['source'] != 'nirmaan-youtube-mobile-macos') return;
...
//       );
//     } catch (e) {
//       debugPrint('MOBILE/MAC YOUTUBE STATE ERROR => $e');
//     }
//   }
//
//   // ── Commands to player (unchanged) ───────────────────────────────────────
...
//     _rotationDebounce?.cancel();
//     if (_isFullscreen && (Platform.isAndroid || Platform.isIOS)) {
...
//         // Disable save / share on the WebView level
...
class _NirmaanYoutubePlayerMobileMacState
  ...
  @override
  void didUpdateWidget(covariant _NirmaanYoutubePlayerMobileMac oldWidget) {
  ...
  // ── Offline HTML5 player — full Nirmaan controls, download blocked ────────
  // Wraps a custom icon in a <span> carrying the required id (and optional
  // hidden state) so the JS show/hide toggles keep working when an icon is
  // swapped. The JS sets element.style.display, which works on <span> too.
  String _wrapId(String id, String inner, {bool hidden = false}) {
    final style = hidden ? ' style="display:none"' : '';
    return '<span id="$id"$style '
        'class="ic-wrap">$inner</span>';
  }
  ...
  String _buildOfflineHtml(int port) {
    ...
    // ── Swappable icons (default = the exact current offline SVGs) ──────────
    // Each can be overridden via offlineIcons (SVG string or image URL).
...
revealControls(true);
  ...
  // ── State handler ─────────────────────────────────────────────────────────
  // The online YouTube HTML sends positionSeconds/durationSeconds (seconds).
  // The offline HTML5 player sends position/duration (milliseconds).
  // Both share the same source name, so we handle both key styles here.
  void _handleState(dynamic raw) {
    try {
      final value = raw is String ? jsonDecode(raw) : raw;
      if (value is! Map) return;
